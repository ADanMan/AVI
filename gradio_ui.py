...
def query_non_stream(
    query: str, use_rag: bool = True, use_safety: bool = True, use_cache: bool = False
) -> tuple[str, dict]:
...
def toggle_rag(enabled: bool) -> str:
    ...
    result = call_api("/api/v1/settings/rag", method="POST", data={"enabled": enabled})
...
def toggle_safety(enabled: bool) -> str:
    ...
    result = call_api("/api/v1/settings/safety", method="POST", data={"mode": mode})
...
def get_rules_list() -> dict:
    ...
    return {
        "total": len(result),
        "rules": result,
        "categories": list({r.get("category", "uncategorized") for r in result}),
    }
...
def create_user_interface():
    ...
    with gr.Column():
        ...
        def user_respond(message, chat_history, stream_enabled, cache_enabled):
            ...
            if stream_enabled:
                # Stream response
                for response, _ in stream_query(
                    message, use_rag=True, use_safety=True, use_cache=cache_enabled
                ):
                    chat_history[-1][1] = response
                    yield chat_history
            else:
                # Non-streaming response
                response, _ = query_non_stream(
                    message, use_rag=True, use_safety=True, use_cache=cache_enabled
                )
                chat_history[-1][1] = response
                yield chat_history
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            with gr.Accordion("🎛️ System Toggles", open=False):
                ...
                rag_toggle.change(toggle_rag, inputs=[rag_toggle], outputs=[rag_status])
                ...
                safety_toggle.change(toggle_safety, inputs=[safety_toggle], outputs=[safety_status])
            ...
            with gr.Accordion("📁 Data Management", open=False):
                ...
                with gr.Row():
                    ...
                    gr.Checkbox(label="Auto-refresh (5s)", value=False, scale=1)

                indexing_status_display = gr.Markdown(
                    value="⚪ No indexing operation running", label="Indexing Progress"
                )
                ...
                def handle_upload(rules_file, docs_file, links_file):
                    upload_msg, _indexing_msg = upload_csv_files(rules_file, docs_file, links_file)
                ...
                check_status_btn.click(
                    check_status, outputs=[indexing_status_display, indexing_metadata]
                )
                ...
                export_btn.click(export_to_csv, outputs=[export_status])
            ...
            with gr.Accordion("📄 View Documents", open=False):
                ...
                view_docs_btn.click(get_documents_list, outputs=[docs_display])
