    ...
    for idx, question in enumerate(tqdm(questions, desc="Фильтрация")):
        try:
            ...
            result_row = {
                ...
                "relevance_scores": "|".join(f"{s:.4f}" for s in relevance_scores)
                if relevance_scores
                else "",
                ...
                "components_vector_rules": filter_result.components_applied.get("vector_rules", False),
    ...
    async def process_question(idx: int, question: str, client: httpx.AsyncClient) -> None:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise Exception(
                    f"API вернул код {response.status_code}: {error_detail}"
                )
            ...
            result_row = {
                ...
                "latency_prompt_modification_ms": input_component_latencies.get("prompt_modification"),
                ...
                "output_latency_output_cleaning_ms": output_component_latencies.get("output_cleaning"),
            ...
            error_msg = str(e)
            logger.error(
                f"Ошибка при API запросе для вопроса {idx}: {error_msg}",
                exc_info=True
            )
...
async def main():
    ...
    if not use_api_mode:
        ...
        try:
            ...
            print(f"✅ Фильтр инициализирован")
