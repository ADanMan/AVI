...
def init_default_api_key() -> str | None:
    ...
    if admin_keys:
        logger.info(
            f"Admin API keys already exist ({len(admin_keys)} active). Skipping default key creation."
        )
    ...
    try:
        ...
        plaintext_key, _api_key = auth_manager.create_api_key(
