...
class ConfigurationTester:
    ...
    async def run_test_scenario(
        self, scenario: TestScenario, config_name: str
    ) -> TestResult:
    ...
    async def benchmark_all(
        ...
        for config_file in config_files:
            config, results = await self.test_configuration(config_file)
...
async def main():
    ...
    parser.add_argument(
        "--config", type=str, help="Path to configuration file to test"
    )
    ...
    parser.add_argument(
        "--output", "-o", type=str, help="Output CSV file for benchmark results"
    )
    ...
    try:
        ...
        if args.config:
            config, results = await tester.test_configuration(args.config, scenarios)
        ...
        # Compare configurations
            ...
            for config_name in args.compare:
                ...
                config, results = await tester.test_configuration(config_file, scenarios)
                metrics = tester.calculate_metrics(results)
                all_metrics.append(metrics)
