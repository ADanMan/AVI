...
def admin_api_key(api_key_manager):
    ...
    plaintext_key, _api_key_obj = api_key_manager.create_api_key(
        name="Test Admin Key", role=Role.ADMIN, metadata={"test": True}
    )
...
def user_api_key(api_key_manager):
    ...
    plaintext_key, _api_key_obj = api_key_manager.create_api_key(
        name="Test User Key", role=Role.USER, metadata={"test": True}
    )
    return plaintext_key
...
def readonly_api_key(api_key_manager):
    ...
    plaintext_key, _api_key_obj = api_key_manager.create_api_key(
        name="Test Readonly Key", role=Role.READONLY, metadata={"test": True}
    )
    return plaintext_key
