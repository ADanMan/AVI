...
class CreateAPIKeyRequest(BaseModel):
    ...
    name: str = Field(..., description="Human-readable name for the key", min_length=1, max_length=100)
    ...
    metadata: dict = Field(default_factory=dict, description="Optional metadata (tags, owner, etc.)")


class CreateAPIKeyResponse(BaseModel):
    """Response model for API key creation."""

    api_key: str = Field(..., description="The plaintext API key (SAVE THIS - it won't be shown again!)")
...
async def revoke_api_key(
    ...
    try:
        ...
        logger.info(f"Admin '{current_api_key.name}' revoked API key with hash '{request.key_hash[:16]}...'")
...
async def delete_api_key(
    ...
    try:
        ...
        logger.info(f"Admin '{current_api_key.name}' deleted API key with hash '{request.key_hash[:16]}...'")
