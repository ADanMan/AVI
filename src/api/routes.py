...
""",
    responses={
        ...
        400: {
            ...
            "content": {"application/json": {"example": {"detail": "No CSV files provided"}}},
        },
        409: {
            "description": "Indexing is already in progress.",
            "content": {
                "application/json": {"example": {"detail": "Reindexing is already in progress"}}
            },
        },
        500: {
            "description": "Failed to process files.",
            "content": {"application/json": {"example": {"detail": "Failed to save CSV files"}}},
        },
    },
)
async def upload_csv_files(
    ...
    rules_file: UploadFile = File(
        ..., description="📋 Safety rules CSV file (filter_rules.csv format)"
    ),
    documents_file: UploadFile = File(
        ..., description="📄 Knowledge documents CSV file (vector_documents.csv format)"
    ),
    links_file: UploadFile = File(
        ..., description="🔗 Rule-document links CSV file (links.csv format)"
    ),
    api_key: APIKey | None = Depends(optional_auth(Role.USER)),
):
    """
    ...
    try:
        ...
        if rules_file:
            if not rules_file.filename.endswith(".csv"):
                raise HTTPException(
                    status_code=400,
                    detail=f"Rules file must be a CSV file, got: {rules_file.filename}",
                )
            ...
            try:
                ...
                if errors:
                    raise HTTPException(
                        status_code=400, detail=f"Rules CSV validation failed: {errors}"
                    )
                ...
                raise HTTPException(status_code=400, detail=f"Invalid rules CSV format: {e!s}")
        ...
        if documents_file:
            if not documents_file.filename.endswith(".csv"):
                raise HTTPException(
                    status_code=400,
                    detail=f"Documents file must be a CSV file, got: {documents_file.filename}",
                )
            ...
            try:
                ...
                if errors:
                    raise HTTPException(
                        status_code=400, detail=f"Documents CSV validation failed: {errors}"
                    )
            except pd.errors.EmptyDataError:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid documents CSV format: {e!s}")
        ...
        if links_file:
            if not links_file.filename.endswith(".csv"):
                raise HTTPException(
                    status_code=400,
                    detail=f"Links file must be a CSV file, got: {links_file.filename}",
                )
            ...
            try:
                ...
                if errors:
                    raise HTTPException(
                        status_code=400, detail=f"Links CSV validation failed: {errors}"
                    )
            except pd.errors.EmptyDataError:
                ...
                raise HTTPException(status_code=400, detail=f"Invalid links CSV format: {e!s}")
...
""",
    responses={
        ...
        500: {
            ...
            "content": {"application/json": {"example": {"detail": "Error during export"}}},
        },
    },
)
...
""",
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {"example": {"detail": "Template must contain {text} variable"}}
            },
        },
    },
)
    ...
    try:
        ...
        if request.use_rag:
            ...
            if request.use_linked_docs and input_filter_result.matches:
                context_docs = await rag_system._get_context_from_matches(
        ...
        final_prompt = settings.PROMPT_MODIFICATION_TEMPLATE.format(
            text=request.query, context=context_string or ""
        )
