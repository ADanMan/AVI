...
class QueryProcessor:
    ...
    async def process_query(self, query: str) -> ProcessedQuery:
        ...
        if query_length <= settings.QUERY_DIRECT_THRESHOLD:
            ...
            return ProcessedQuery(search_queries=[query], full_context=query, strategy="direct")

        else:
            # Long query - chunk and extract search queries
            logger.info(f"Using chunking strategy (query length > 8K: {query_length} chars)")
    ...
    def _select_search_queries(self, chunks: list[str]) -> list[str]:
        ...
        logger.info(f"Selected {len(selected)} representative chunks from {len(chunks)} total")
        return selected
