...
_indexing_duration_metric = Histogram(
    ...
    labelnames=("operation", "status"),  # operation: reindex_all, export_csv, etc. / status: success, error
    namespace=_prometheus_namespace,
...
class ContentFilterMetrics:
    ...
    def record_indexing_operation(
        ...
        _indexing_duration_metric.labels(operation=operation, status=status).observe(duration_seconds)
